    ...
    private static final class SecureNode {
        ...
        private SecureNode(SecureAlgorithm secureAlgorithm, byte[] dataBytes) {
            ...
            switch (this.secureAlgorithm) {
                ...
                case AES128, AES192, AES256, DES, TRIPLE_DES, SM4 -> {
        ...
        private SecureProvider initCryptor(boolean encrypt) {
            ...
            if (this.initialized) {
                try {
                    switch (this.secureAlgorithm) {
                        case RSA1024, RSA2048 -> secureProvider = encrypt ? SecurityUtils.RSAEncryptor(this.publicKey)
                                : SecurityUtils.RSADecryptor(this.privateKey);
                        case SM2 -> secureProvider = encrypt ? SecurityUtils.SM2Encryptor(this.publicKey)
                        ...
                        case DES -> secureProvider = encrypt ? SecurityUtils.DESEncryptor(this.keyBytes)
                                : SecurityUtils.DESDecryptor(this.keyBytes);
                        case TRIPLE_DES -> secureProvider = encrypt ? SecurityUtils.TripleDESEncryptor(this.keyBytes)
                                : SecurityUtils.TripleDESDecryptor(this.keyBytes);
                        case SM4 -> secureProvider = encrypt ? SecurityUtils.SM4Encryptor(this.keyBytes)
