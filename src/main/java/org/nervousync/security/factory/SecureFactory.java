    ...
    private static final class SecureNode {
        ...
        private SecureAdapter initCryptor(boolean encrypt) {
            ...
            if (this.initialized) {
                try {
                    switch (this.secureAlgorithm) {
                        case RSA1024, RSA2048 -> secureAdapter = encrypt ? SecurityUtils.RSAEncryptor(this.publicKey)
                                : SecurityUtils.RSADecryptor(this.privateKey);
                        case SM2 -> secureAdapter = encrypt ? SecurityUtils.SM2Encryptor(this.publicKey)
                        ...
                        case DES -> secureAdapter = encrypt ? SecurityUtils.DESEncryptor(this.keyBytes)
                                : SecurityUtils.DESDecryptor(this.keyBytes);
                        case TRIPLE_DES -> secureAdapter = encrypt ? SecurityUtils.TripleDESEncryptor(this.keyBytes)
                                : SecurityUtils.TripleDESDecryptor(this.keyBytes);
                        case SM4 -> secureAdapter = encrypt ? SecurityUtils.SM4Encryptor(this.keyBytes)
