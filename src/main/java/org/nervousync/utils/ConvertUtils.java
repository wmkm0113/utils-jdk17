...
public final class ConvertUtils {
	...
	public static List<Object> collectionToList(final Object collection) {
		...
		} else if (collection instanceof Enumeration<?> enumeration) {
			list = new ArrayList<>();
			while(enumeration.hasMoreElements()) {
				list.add(enumeration.nextElement());
			}
		} else if (collection instanceof Iterator<?> iterator) {
			list = new ArrayList<>();
	...
	private static boolean storeProperties(final Properties properties, final String propertiesFilePath,
		...
		try {
			...
			switch (fileExtName.toLowerCase()) {
				case "xml" -> properties.storeToXML(fileOutputStream, comment, Globals.DEFAULT_ENCODING);
				case "properties" -> properties.store(fileOutputStream, comment);
				default -> throw new Exception("Properties file error");
			}
			return true;
		} catch (Exception e) {
			if (LOGGER.isDebugEnabled()) {
