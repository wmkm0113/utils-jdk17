...
public final class OTPUtils {
	...
	private static int generateCode(final CalcType calcType, final String secret, long randomCode) {
		...
		byte[] hash;
		switch (calcType) {
			case HmacSHA1 -> hash = SecurityUtils.HmacSHA1(secretBytes, signData);
			case HmacSHA256 -> hash = SecurityUtils.HmacSHA256(secretBytes, signData);
			case HmacSHA512 -> hash = SecurityUtils.HmacSHA512(secretBytes, signData);
			default -> {
				return Globals.DEFAULT_VALUE_INT;
			}
		}
