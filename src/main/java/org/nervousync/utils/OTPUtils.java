...
public final class OTPUtils {
	...
	private static int generateCode(final CalcType calcType, final String randomKey, long calcTime) {
		...
		byte[] hash;
		switch (calcType) {
			case HmacSHA1 -> hash = SecurityUtils.HmacSHA1(secret, signData);
			case HmacSHA256 -> hash = SecurityUtils.HmacSHA256(secret, signData);
			case HmacSHA512 -> hash = SecurityUtils.HmacSHA512(secret, signData);
			default -> {
				return Globals.DEFAULT_VALUE_INT;
			}
		}
