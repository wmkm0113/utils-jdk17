...
public final class RawUtils {
	...
	private static void writeNumber(final byte[] dataBytes, final int position, final int dataSize,
		...
		switch (dataSize) {
			case Short.SIZE -> byteBuffer.putShort((short) value);
			case Integer.SIZE -> byteBuffer.putInt((int) value);
			case Long.SIZE -> byteBuffer.putLong((long) value);
			default -> throw new DataInvalidException("Unknown data length");
		}
//		byte[] numberBytes = byteBuffer.order(byteOrder).array();
//		System.arraycopy(numberBytes, Globals.INITIALIZE_INT_VALUE, dataBytes, position, dataLength);
	}
